...
def format_alert_email(payload: Dict[str, Any]) -> str:
    ...
    lines = []
    lines.append(f"Time: {g('time','Time','asof','AsOf')}")
    lines.append(f"Symbol: {g('symbol','Symbol')}")
    lines.append(
        f"Bias: {g('bias','Bias')}   Score: {g('score','Score')}   Session: {g('session','Session')}"
    )
    lines.append("")
    lines.append(f"Last: {g('last','Last')}")
    lines.append(f"Entry: {g('entry','Entry')}")
    lines.append(f"Stop: {g('stop','Stop')}")
    lines.append(f"TP1: {g('tp1','TP1')}")
    lines.append(f"TP2: {g('tp2','TP2')}")
    lines.append("")
